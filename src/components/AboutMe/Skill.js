    ...
    if (Array.isArray(picture)) {
        return (
            <div className='d-flex'>
                {
                    picture.map(pic => (
                        <img className='skill-icon me-1' src={require(`./skill_pic/${pic}`)} />
                    ))
                }
            </div>
        )
    ...
    if (Array.isArray(proficiency)) {
            ...
            <>
                {
                    proficiency.map((pro, index) => (
                        <div class={`progress ${index > 0 ? 'mt-1' : ''}`} style={{ height: '10px' }}>
                            <div class={`progress-bar progress-bar-striped bg-${pro >= 75 ? 'danger' : pro >= 50 ? 'warning' :
        ...
        return (
            <div class="progress" style={{ height: '10px' }}>
                <div class={`progress-bar progress-bar-striped bg-${proficiency >= 75 ? 'danger' : proficiency >= 50 ? 'warning' :
